...
function tournamentReducer(state, action) {
  switch (action.type) {
    ...
    case 'SET_TOURNAMENTS':
      return { ...state, registeredTournaments: action.payload };
    case 'SET_ARCHERS':
      return { ...state, archers: action.payload };
...
function App() {
  ...
  useEffect(() => {
    const fetchTournaments = async () => {
      try {
        ...
        dispatch({ type: 'SET_TOURNAMENTS', payload: response.data });
      } catch (error) {
  ...
  // Update selected tournament in local storage
  useEffect(() => {
  ...
  // Update tournament info when selected tournament changes
  useEffect(() => {
    ...
    if (tpl && tpl.data) {
      ...
      allowedKeys.forEach(k => { 
        if (typeof tpl.data[k] !== 'undefined') payload[k] = tpl.data[k]; 
      });
  ...
  if (tournamentState.error) {
    return (
      <div className="p-4 text-red-600">
        <p>エラーが発生しました: {tournamentState.error}</p>
        <button 
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-white shadow-sm">
        <div className="max-w-7xl mx-auto px-4 py-4 sm:px-6 lg:px-8 flex justify-between items-center">
          <h1 className="text-xl font-semibold text-gray-900">弓道大会運営システム</h1>
          ...
          {isAdminLoggedIn ? (
            ...
            <button
              onClick={() => setAdminLoginStep('password')}
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 sm:px-6 lg:px-8">
        {adminLoginStep === 'password' && (
          <AdminLoginView
            ...
            onClose={() => setAdminLoginStep(null)}
          />
        )}
        ...
        {!isArcherVerified ? (
            ...
            <div className="login-container">
              <div className="login-box">
                ...
                <input
                  ...
                  onChange={(e) => setArcherIdInputModal(e.target.value)}
                  placeholder="選手IDを入力"
                  className="input-field"
                />
                {archerIdError && <p className="error-message">{archerIdError}</p>}
                <button 
                  onClick={() => {
                    ...
                    const pick = tournamentState.registeredTournaments.find(t => val.startsWith(t.id));
                    if (!pick) {
          ...
          <div>
            <nav className="flex space-x-4 border-b border-gray-200 mb-6">
              <button
                onClick={() => setMainView('tournament')}
                className={`${mainView === 'tournament' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm`}
              >
                大会
              </button>
              <button
                onClick={() => setMainView('checkin')}
                className={`${mainView === 'checkin' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm`}
              >
                受付
              </button>
              <button
                onClick={() => setMainView('ranking')}
                className={`${mainView === 'ranking' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm`}
              >
                結果
              </button>
            </nav>
      ...
      <footer className="bg-white border-t border-gray-200 mt-8">
        <div className="max-w-7xl mx-auto px-4 py-6 sm:px-6 lg:px-8">
          <p className="text-center text-sm text-gray-500">
            &copy; {new Date().getFullYear()} 弓道大会運営システム. All rights reserved.
          </p>
        </div>
      </footer>
    </div>
...
export default App;
  ...
  const isPassed = useCallback((archer) => {
    ...
    const currentRound = tournamentState.tournament.currentRound || 1;
    const isFirstRound = currentRound === 1;
    const endIndex = isFirstRound ? tournamentState.tournament.arrowsRound1 : (tournamentState.tournament.arrowsRound1 + tournamentState.tournament.arrowsRound2);
    ...
    // 大会設定の通過ルールに基づいて判定
    const passRule = tournamentState.tournament.passRule || 'all_four';
    const currentRoundArrows = isFirstRound ? tournamentState.tournament.arrowsRound1 : tournamentState.tournament.arrowsRound2;
  ...
  const passedArchers = useMemo(() => {
    return tournamentState.archers.filter(archer => isPassed(archer) === true);
  }, [tournamentState.archers, isPassed]);
  ...
  const printProgram = () => {
    ...
    const pages = Math.max(1, Math.ceil(tournamentState.archers.length / perPage));
    ...
    for (let p = 0; p < pages; p++) {
      ...
      const end = Math.min(start + perPage, tournamentState.archers.length);
      for (let i = start; i < end; i++) {
        const a = tournamentState.archers[i];
...
// Component exports are now in their respective files
